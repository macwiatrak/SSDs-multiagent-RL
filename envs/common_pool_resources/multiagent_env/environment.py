...
class GameEnv:
    def __init__(self, width=24, height=12, agent_hidden=25):
        self.size_x = width
        self.size_y = height
        self.objects = []
    ...
    def reset(self):
        self.agent1 = AgentObj(coordinates=(1, 0), type=0, name='agent1', direction=0)
        self.agent2 = AgentObj(coordinates=(2, 2), type=0, name='agent2', direction=3)
        self.agent3 = AgentObj(coordinates=(3, 1), type=0, name='agent3', direction=3)
        self.agent4 = AgentObj(coordinates=(2, 0), type=0, name='agent4', direction=0)


        agent_list = [self.agent1, self.agent2, self.agent3, self.agent4]
        ...
        self.agent4_actions = [self.agent4.move_forward, self.agent4.move_backward, self.agent4.move_left,
                               self.agent4.move_right,
                               self.agent4.turn_left, self.agent4.turn_right, self.agent4.beam, self.agent4.stay]

        self.agent1_beam_set = []
        self.agent2_beam_set = []
        self.agent3_beam_set = []
        self.agent4_beam_set = []

        beam_set_list = self.agent1_beam_set + self.agent2_beam_set + self.agent3_beam_set + self.agent4_beam_set
        ...
        for agent in agent_list:
            grid_reset[agent.get_front_player(env_x_size=24,
                                              env_y_size=12)[0]][agent.get_front_player(env_x_size=24,
                                              env_y_size=12)[1]] = CellType.AGENT_FRONT
            grid_reset[agent.x][agent.y] = CellType.OPPONENT
        if beam_set_list:
            for beam in beam_set_list:
                grid_reset[beam[0]][beam[1]] = CellType.BEAM

        agent1_obs = self.agent1.partial_observation(env_x_size=24, env_y_size=12, grid=grid_reset, obs_rows=7,
                                                     obs_cols=15)
        agent2_obs = self.agent2.partial_observation(env_x_size=24, env_y_size=12, grid=grid_reset, obs_rows=7,
                                                     obs_cols=15)
        agent3_obs = self.agent3.partial_observation(env_x_size=24, env_y_size=12, grid=grid_reset, obs_rows=7,
                                                     obs_cols=15)
        agent4_obs = self.agent4.partial_observation(env_x_size=24, env_y_size=12, grid=grid_reset, obs_rows=7,
                                                     obs_cols=15)
        return [convert_observation_to_rgb(agent1_obs), convert_observation_to_rgb(agent2_obs),
                convert_observation_to_rgb(agent3_obs), convert_observation_to_rgb(agent4_obs)]
    ...
    def step(self, action_n):
        ...
        assert action_n[2] in range(8), 'agent2 take wrong action'
        assert action_n[3] in range(8), 'agent2 take wrong action'
        ...
        agent4_old_x, agent4_old_y = self.agent4.x, self.agent4.y
        ...
        if not self.agent4.is_hidden():
            untagged += 1
            agent4_action_return = self.agent4_actions[action_n[3]](env_x_size=self.size_x, env_y_size=self.size_y)
            self.agent4_beam_set = [] if action_n[3] != 6 else agent4_action_return
        ...
        if not self.agent1.is_hidden() and not self.agent4.is_hidden() and \
                ((self.agent1.x == self.agent4.x and self.agent1.y == self.agent4.y) or
                 (self.agent1.x == agent4_old_x and self.agent1.y == agent4_old_y and
                  self.agent4.x == agent1_old_x and self.agent4.y == agent1_old_y)):
            self.agent1.x, self.agent1.y = agent1_old_x, agent1_old_y
            self.agent4.x, self.agent4.y = agent4_old_x, agent4_old_y
        if not self.agent2.is_hidden() and not self.agent3.is_hidden() and \
            ...
            self.agent2.x, self.agent2.y = agent2_old_x, agent2_old_y
            self.agent3.x, self.agent3.y = agent3_old_x, agent3_old_y
        if not self.agent2.is_hidden() and not self.agent4.is_hidden() and \
                ((self.agent2.x == self.agent4.x and self.agent2.y == self.agent4.y) or
                 (self.agent2.x == agent4_old_x and self.agent2.y == agent4_old_y and
                  self.agent4.x == agent2_old_x and self.agent4.y == agent2_old_y)):
            self.agent2.x, self.agent2.y = agent2_old_x, agent2_old_y
            self.agent4.x, self.agent4.y = agent4_old_x, agent4_old_y
        if not self.agent3.is_hidden() and not self.agent4.is_hidden() and \
                ((self.agent3.x == self.agent4.x and self.agent3.y == self.agent4.y) or
                 (self.agent3.x == agent4_old_x and self.agent3.y == agent4_old_y and
                  self.agent4.x == agent3_old_x and self.agent4.y == agent3_old_y)):
            self.agent3.x, self.agent3.y = agent3_old_x, agent3_old_y
            self.agent4.x, self.agent4.y = agent4_old_x, agent4_old_y
        ...
        for food in self.food_objects:
            ...
            if not food.is_collected:
                ...
                elif not self.agent4.is_hidden() and food.x == self.agent4.x and food.y == self.agent4.y:
                    agent4_reward = food.eat()
        ...
        for agent in agent_list:
            if not agent.is_hidden():
                grid_step[agent.get_front_player(env_x_size=24,
                                                 env_y_size=12)[0]][agent.get_front_player(env_x_size=24,
                                                 env_y_size=12)[1]] = CellType.AGENT_FRONT
        ...
        agent1_obs = self.agent1.partial_observation(env_x_size=24, env_y_size=12, grid=grid_step, obs_rows=7,
                                                     obs_cols=15)
        agent2_obs = self.agent2.partial_observation(env_x_size=24, env_y_size=12, grid=grid_step, obs_rows=7,
                                                     obs_cols=15)
        agent3_obs = self.agent3.partial_observation(env_x_size=24, env_y_size=12, grid=grid_step, obs_rows=7,
                                                     obs_cols=15)
        agent4_obs = self.agent4.partial_observation(env_x_size=24, env_y_size=12, grid=grid_step, obs_rows=7,
                                                     obs_cols=15)
        rew_n = [agent1_reward, agent2_reward, agent3_reward, agent4_reward]
    ...
    def contribute_matrix(self):
        ...
        for x, y in self.agent3_beam_set:
            ...
            a[y + 1, x + 1, 1] = 1
            a[y + 1, x + 1, 2] = 1
        for x, y in self.agent4_beam_set:
            a[y + 1, x + 1, 0] = 1
            a[y + 1, x + 1, 1] = 1
            a[y + 1, x + 1, 2] = 1
        ...
        for i in range(3):
            ...
            if not self.agent4.is_hidden():
                delta_x, delta_y = self.agent4.move_forward_delta()
                a[self.agent4.y + 1 + delta_y, self.agent4.x + 1 + delta_x, i] = 0.5
            if not self.agent1.is_hidden():
            ...
            if not self.agent4.is_hidden():
                a[self.agent4.y + 1, self.agent4.x + 1, i] = 1 if i == self.agent4.type else 0
